...
class DQNAgent:
    def __init__(self, state_size, action_size):
        ...
        # ОПТИМИЗИРОВАННЫЕ ПАРАМЕТРЫ ДЛЯ 16×16
        self.memory = deque(maxlen=5000)  # Увеличили буфер
        self.gamma = 0.95
        self.epsilon = 1.0
        self.epsilon_min = 0.05  # Больше исследования для сложного лабиринта
        self.epsilon_decay = 0.998  # Медленнее уменьшаем exploration
        self.learning_rate = 0.0005  # Меньше learning rate

        # ОПТИМАЛЬНАЯ АРХИТЕКТУРА ДЛЯ 16×16
        self.model = self._build_optimized_model()
        self.target_model = self._build_optimized_model()
        self.update_target_model()

        self.batch_size = 64  # Увеличили batch size
    ...
    def _build_optimized_model(self):
        ...
        model = models.Sequential([
            # Увеличенная, но РАЗУМНАЯ архитектура
            layers.Dense(128, activation='relu', input_shape=(self.state_size,),
                         kernel_regularizer=regularizers.l2(0.001)),
            layers.Dropout(0.2),  # Добавили dropout против переобучения
    ...
    def act(self, state, agent_pos=None, maze=None):
        ...
        if np.random.random() <= self.epsilon:
            # "УМНОЕ" исследование для больших лабиринтов
    ...
    def _smart_exploration(self, agent_pos, maze):
        ...
        # Проверяем возможные направления (избегаем стен)
        directions = [
            (0, -1, 0),  # вверх
            (1, 0, 1),  # вправо
            (0, 1, 2),  # вниз
        ...
        # Предпочитаем непосещенные направления
        if possible_actions:
    ...
    def remember(self, state, action, reward, next_state, done):
        ...
        # Нормализуем форматы перед сохранением
        state = self._normalize_state(state)
    ...
    def end_episode(self):
        ...
        # Очищаем историю для нового эпизода
